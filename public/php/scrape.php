...
define('SCRAPE_TIMEOUT_DEFAULT_MS', 8000);
define('SCRAPE_TIMEOUT_MIN_MS', 500);
define('SCRAPE_TIMEOUT_MAX_MS', 30000);

$url = $_GET['url'];
$regexp = $_GET['regexp'];
$timeout_ms = SCRAPE_TIMEOUT_DEFAULT_MS;

if (isset($_GET['timeout_ms']) && is_numeric($_GET['timeout_ms'])) {
    $timeout_ms = (int) $_GET['timeout_ms'];
}

$timeout_ms = max(SCRAPE_TIMEOUT_MIN_MS, min(SCRAPE_TIMEOUT_MAX_MS, $timeout_ms));
$connect_timeout_ms = min($timeout_ms, 2000);
